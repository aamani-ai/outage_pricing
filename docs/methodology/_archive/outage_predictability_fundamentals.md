...
> from — the shapes overlapped in practice (bidirectional label churn). The descriptive dashboard
> view may still render, but the **direction is now regime routing**:
> [`OUTAGE_MODELING_FRAMEWORK.md`](../../OUTAGE_MODELING_FRAMEWORK.md) Step 3 ▸ *Reframe — REGIME
> ROUTING*. Kept for context, not as the build target.
