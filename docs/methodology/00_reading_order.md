...
4. **[Per-Customer Pricing](02_per_customer/per_customer_pricing_fundamentals.md)** — the headline rate. Why it's 30–100× smaller than v0. The single load-bearing assumption ([A011](assumptions.md)).
   - **[The Customer-Base Denominator](02_per_customer/customer_base_denominator_fundamentals.md)** — what the per-customer *denominator* actually is: the menu of candidate bases (MCC / households / housing units / peak / EIA-861), why a "customer" isn't one clean thing, why we use the composite `max(MCC, housing_units, peak)` ([A018](assumptions.md)), and the business-side nuance still open.
...
9. *(superseded — context only)* the old descriptive layer, kept bannered: [Outage Trend](03_risk_clustering/outage_trend_fundamentals.md) (slope is reused as a regime feature) · [Predictability](_archive/outage_predictability_fundamentals.md) · [Lambda Shadow](_archive/lambda_shadow_pricing_fundamentals.md) (the price-move is now Step-5-gated). The 7-shape labels were replaced by the 5 outcomes above.
