# Status across the full pipeline
...
This is the single scan page for where each outage-pricing layer stands now.
It is intentionally status-first: it separates what is active, what is under
review, what is still work in progress, and what remains in discussion.
...
| Status | Meaning |
|---|---|
| **Done · active** | Discussed, verified, and currently part of the active headline pricing read |
| **Under review** | Design or first evidence exists, but the layer is not yet an active premium move |
| **Work in progress** | The lane is important and scoped, but the data/model evidence is still being built |
| **In discussion** | The option space is framed, but external/vendor/data decisions remain |

Important nuance: **under review** can mean the dashboard already shows the
layer. It does **not** mean the layer mutates active premium. Predictability is
visible as a statistical routing and shadow read, but it needs verification
before final price implementation.

## Where Each Layer Stands

| Layer | Current status | Read |
|---|---|---|
| Customer basis risk | **Done · active** | Done - discussed with the team and verified. Active headline rate. |
| Location basis risk | **Under review** | Design and pilot evidence are in; targeting completion this week. |
| Predictability routing (statistically) | **Under review** | Initial forward-looking layer is done; shadow, not active. |
| Hazard & weather + grid condition | **Work in progress** | Advanced cause-attribution lanes. |
| Trigger source | **In discussion** | Option space framed; vendor diligence ahead. |

**Sequencing principle:** basis-risk alignment is closed before
forward-looking signal enters pricing; forward-looking signal is validated on
holdout years before it moves an active premium.

## 1. Baseline Pricing

### County empirical baseline - **done / active**
...
### Customer basis risk - **done / active**
...
### Predictability routing and shadow frequency read - **under review**
...
### Trigger source alignment - **in discussion**
