...
*Audience: senior team. Last reviewed: 2026-06-20. Reads naturally after [`event_catalog_fundamentals.md`](../01_eventization/event_catalog_fundamentals.md).*

> **Note (2026-06-22).** The trend **slope / t_stat** computed here stays useful — it's a candidate
> **regime feature**. But the worsening / improving / stable **class as a deliverable** is folded
> into **regime routing** (the slope feeds regime identity; it isn't the product itself), and it must
> run on the **masked** series (coverage ramp removed) — see
> [`05_source_coverage_mask.md`](../../dicsscssion/eventization_frequency_contract/05_source_coverage_mask.md).
> Direction: [`OUTAGE_MODELING_FRAMEWORK.md`](../../OUTAGE_MODELING_FRAMEWORK.md) Step 3 ▸ *Reframe — REGIME ROUTING*.
...
Counties showing **red** could be real climate / grid trend, could be coverage drift, or both. We cannot separate from EAGLE-I alone — see the [Outage Trend Validation Plan](../../plan/03_risk_clustering/outage_trend_validation_plan.md) for the tracked work to do that separation.
...
- Forward modifier plan: [`docs/plan/forward_looking_modeling_plan.md`](../../plan/05_forward_regime/forward_looking_modeling_plan.md)
